...
class OrdersController < ApplicationController
  ...
  def new
    ...
    @products = @cart.cart_items.map do |item|
      { name: item.part.name, description: item.part.description, weight: item.part.weight, quantity: item.quantity }
    end
  ...
  def add_part
    ...
    @selected_system = System.find(params[:system_id]) if params[:system_id].present?
  ...
  def order_params
    params.require(:order).permit(:building_site, :delivery_date, :info, :building_site_info, :new_delivery_date,
                                  :car_number, :status, :storage_info, :checkbox, :send_email, order_lists_attributes: %i[part_id quantity description delivery_quantity checkbox])
  end
